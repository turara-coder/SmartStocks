...
#### データベース環境構築
- [ ] **1.2.3** 開発環境データベース構築
  - [ ] PostgreSQL セットアップ（Docker使用推奨）
  - [ ] Redis セットアップ（Docker使用推奨）
  - [ ] 接続設定・接続テスト
  ...
  **🤖 [1.2.3] AI実装プロンプト**:
  ```
  DockerでローカルデータベースとRedis環境を構築：
  1. docker-compose.ymlを作成：
     - PostgreSQL 15（ユーザー名/パスワード/DB名設定）
     - Redis 7（永続化設定）
     - ヘルスチェック設定
     - ボリューム設定（データ永続化）
  2. .envファイルの設定例：
     - DATABASE_URL="postgresql://username:password@localhost:5432/smartstocks"
     - REDIS_URL="redis://localhost:6379"
  3. 接続テスト用のスクリプト作成（Prisma接続確認 + Redis接続確認）
  4. データベース起動・停止用のnpm scriptsを追加
  ```
...
#### 外部API調査・準備
- [ ] **1.4.1** データソース選定・準備
  - [ ] Alpha Vantage API キー取得・動作確認
  - [ ] Yahoo Finance API 調査・実装方法確認
  - [ ] 日本取引所グループAPI調査
  - [ ] レート制限・料金プラン確認・文書化
  - **完了日**: _____ **担当者**: _____
...
#### キャッシュ・データ管理システム
- [ ] **1.4.3** Redis キャッシュシステム実装
  - [ ] Redis 接続設定
  - [ ] キャッシュキー設計実装
  - [ ] TTL設定（株価データ: 5分, その他設定に応じて）
  - [ ] キャッシュ無効化戦略実装
  - **完了日**: _____ **担当者**: _____
...
**🤖 AI実装プロンプト**:
```
Next.js API Routesで株価データ取得APIを実装：
1. `app/api/stocks/search/route.ts`：
   - クエリパラメータ検証（Zod使用）
   - 複数データソース対応（Alpha Vantage, Yahoo Finance）
   - Redisキャッシュ実装（5分TTL）
   - レート制限実装
...
4. 共通ユーティリティ：
   ...
   - データ正規化関数
   - キャッシュヘルパー
docs/api-specification.mdの仕様に完全準拠した実装を行う
```
...
#### 認証システム構築
- [ ] **2.3.1** NextAuth.js セットアップ
  - [ ] NextAuth.js インストール・基本設定
  - [ ] プロバイダー設定（Email, Google OAuth）
  - [ ] セッション管理設定
  - [ ] ミドルウェア設定（認証ガード）
  - **完了日**: _____ **担当者**: _____
...
- [ ] **4.2.2** AI生成システム統合
  ...
  **🤖 [4.2.2] AI実装プロンプト**:
  ```
  OpenAI GPT APIを使ったれいのセリフ生成システムを実装：
  1. GPT API クライアント設定：
     - OpenAI APIキー管理
     - レート制限・エラーハンドリング
     - タイムアウト設定
...
#### 本番環境構築・デプロイ
- [ ] **6.2.1** インフラ・デプロイ準備
  - [ ] 本番環境セットアップ 📖 *参照: `README.md` (インフラ・デプロイ)*
  - [ ] SSL証明書・ドメイン設定
  - [ ] 監視システム・ログ設定 📖 *参照: `README.md` (運用・保守)*
  - [ ] バックアップシステム構築 📖 *参照: `docs/database-schema.md` (バックアップ戦略)*
  - [ ] CI/CD パイプライン本番対応
  - **完了日**: _____ **担当者**: _____
